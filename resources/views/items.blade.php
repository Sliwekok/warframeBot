    ...
    @if(count($watched) > 0)
        ...
        @foreach($watched as $order)
            <div class="col-3 orderPanel">
                <div class="col-12"><p class="itemname"><a href="{{url("/search/$order->name")}}">{{$order->name}}</a></p></div>
                ...
                <div class="col-3 offset-9 orderMenagment">
                    ...
                    <div class="col-6"><span class="editOrder delete deleteWatched" data-item='{{$order->name}}' data-platform='{{$order->platform}}' data-toggle="tooltip" data-bs-placement="top" title="Edit price of order"><i class="icon icon-cancel"></i></span></div>
                </div>
            </div>
        @endforeach
    @else
